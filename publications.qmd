...
# Install if needed: install.packages("scholar")
if(!require(scholar)) install.packages('scholar')
require(scholar)

user_id <- "GFxNB78AAAAJ"
profile <- get_profile(user_id)

# Display key metrics
total <- profile$total_cites
...
I have authored 102 research articles indexed in ISI and SCOPUS, with additional manuscripts presently under review. These manuscripts have reached `r total` citations according to [Google Scholar](https://scholar.google.com.co/citations?user=GFxNB78AAAAJ&hl=en) as of `r month` `r day`, `r year`.
...
<!-- As of `r month` `r day`, `r year`, these manuscripts have reached `r total` citations and an $h_{\text{index}}$ of `r hindex` according to [Google Scholar](https://scholar.google.com.co/citations?user=GFxNB78AAAAJ&hl=en).  -->
